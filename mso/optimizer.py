...
class BasePSOptimizer:
    ...
    def __init__(self, swarms, inference_model, scoring_functions=None):
        ...
        self.func_names = [func.name for func in scoring_functions]
        self.swarms = swarms
    ...
    def _update_best_solutions(self, num_track):
        ...
        for name in self.func_names:
            new_df[name] = [score for swarm in self.swarms for score in swarm.unscaled_scores[name]]
    ...
    def _update_best_fitness_history(self, step):
        ...
        :return: None
        """
        new_df = pd.DataFrame(columns=["step", "swarm", "smiles", "fitness",])
